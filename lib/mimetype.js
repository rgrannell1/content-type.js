...

{
	...
	(function () {
		...
		var type = zipKeys([["/", "_SLASH"]].concat(tokenChar.map(function (char) {
			return [char, "type"];
		})));

		var subtype = zipKeys([[";", "_SPACE"]].concat(tokenChar.map(function (char) {
			return [char, "subtype"];
		})));

		var attribute = zipKeys([["=", "_EQUAL"]].concat(tokenChar.filter(function (char) {
			return control.indexOf(char) === -1;
		}).map(function (char) {
			return [char, "attribute"];
		})));

		var singleQuoted = zipKeys([["'", "_SPACE"]].concat(tokenChar.concat(tspecials).concat(" ").filter(function (char) {
			return char != "'";
		}).map(function (char) {
			return [char, "single-quoted"];
		})));

		var doubleQuoted = zipKeys([["\"", "_SPACE"]].concat(tokenChar.concat(tspecials).concat(" ").filter(function (char) {
			return char != "\"";
		}).map(function (char) {
			return [char, "double-quoted"];
		})));

		var unquoted = zipKeys([[";", "_SPACE"]].concat(tokenChar.map(function (char) {
			return [char, "unquoted"];
		})));

		var _SLASH = zipKeys(tokenChar.map(function (char) {
			return [char, "subtype"];
		}));
